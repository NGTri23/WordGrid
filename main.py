...
""" Fichier main.py """
""" Contient :
    - les fonctions pour l'enregistrement du score en mode Chrono
    - les view fonctions pour les @app.route
"""
...
def save_best_score_txt(mot: str, difficulte: int, tentatives_utilisees: int, temps_sec: float):
    """ Fonction permettant d'écrire et modifier le contenu du fichier 'leaderboard.txt'
        Pour mettre à jour les scores faits lors du mode 'Chrono'
    """
...
def load_scores_txt(limit: int = 20) -> list:
    """ Fonction permettant d'obternier les 'limit' premiers meilleurs scores pour afficher sur le LeaderBoard.
        Elle retourne la liste de scores
    """
    if not os.path.exists(LEADERBOARD_FILE):
        return []
    ...
    with open(LEADERBOARD_FILE, "r", encoding="utf-8") as f:
        for line in f:
            ...
            scores.append({
                "mot": mot,
                "difficulte": liste_difficulte.get(10-int(difficulte), "Inconnue"),
...
def menu_config():
    ...
    session.clear() # supprimer toute la cache des sessions
...
def game():
    ...
    if "mot_secret" not in session :
        session["mot_secret"] = choix_dico(configuration) #  stocker le mot secret jusqu'à la fin de la partie
    ...
    if "essais_restant" not in session :
